            ...
            <div style="background-color: #fff; border-radius: 2px" id="collapseTwo" class="panel-collapse collapse">
                <div class="box-body">
                    <div class="col-md-offset-3 col-md-6">
                        <div class="box-body">
                            <h4>Assign New Status</h4>
                            <form action="/changeStatus" method="post">
                                {{ csrf_field() }}
                                <input type="hidden" name="prevStatus" value="{{ $orderDetail[0]->o_status }}" readonly>
                                <input type="hidden" name="fabric" value="{{ $orderDetail[0]->o_fabric }}" readonly>
                                <input type="hidden" name="lining" value="{{ $orderDetail[0]->o_lining }}" readonly>
                                <input type="hidden" name="itemType" value="{{ $orderDetail[0]->item_type }}" readonly>
                                <input type="hidden" name="orderId" value="{{ $orderDetail[0]->o_id }}">
                                <select class="form-control" name="status">
                                    @if($orderDetail[0]->o_status != 'Pending Review' )
                                        <option value="Pending Review">Pending Review</option>
                                    @endif
                                    @if($orderDetail[0]->o_status != 'Pre-production' )
                                        <option value="Pre-production">Pre-production</option>
                                    @endif
                                    @if($orderDetail[0]->o_status != 'Canceled' )
                                        <option value="Canceled">Canceled</option>
                                    @endif
                                    @if($orderDetail[0]->o_status != 'In Production' )
                                        <option value="In Production">In Production</option>
                                    @endif
                                    @if($orderDetail[0]->o_status != 'In Warehouse' )
                                        <option value="In Warehouse">In Warehouse</option>
                                    @endif
                                    @if($orderDetail[0]->o_status != 'In Store' )
                                        <option value="In Warehouse">In Store</option>
                                    @endif
                                </select>
                                <h4>Enter password</h4>
                                <input required type="password" name="adminPassword" class="form-control" placeholder="Enter admin password to change status">
                                <h4></h4>
